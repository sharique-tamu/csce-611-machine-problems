...
SimpleFramePool::FrameState
SimpleFramePool::get_state(unsigned long _frame_no) {
  unsigned int bitmap_index = _frame_no / 8;
  unsigned char mask = 0x1 << (_frame_no % 8);

  return ((bitmap[bitmap_index] & mask) == 0) ? FrameState::Used
                                              : FrameState::Free;
}
...
unsigned long SimpleFramePool::get_frame() {
  ...
  // We don't need to check whether we overrun. This is handled by
  // assert(nFreeFrame>0) above.
...
void SimpleFramePool::release_frame(unsigned long _frame_no) {
  ...
  //    NOTE: Keep in mind that you first need to identify the correct frame
  //    pool. The function is static, and you are only given a frame number. You
  //    do have to figure out which frame pool this frame belongs to before you
  //    can call the frame-pool-specific release_frame function.
